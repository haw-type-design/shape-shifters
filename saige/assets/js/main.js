document.addEventListener("DOMContentLoaded", () => {
    ...
    for (const [key, value] of Object.entries(storageItems)) {
        const htmlElement = document.querySelector(`article#${key}`);
    ...
    if (document.body.classList.contains('toc')) {
        articles.forEach((article) => {
            article.querySelector('a').onclick = (e) => {
                const id = article.id;
    ...
    // Dynamically insert real link
    const
    visited = document.querySelectorAll('.visited'),
          lastVisited = visited[visited.length - 1],
          successor = lastVisited.nextElementSibling,
          successorLink = successor.querySelector('a'),
          successorLinkAttr = successorLink.getAttribute('data-href');


    // if visited --> keep visitable
    visited.forEach((visit) => {
        visit.querySelector('a').href = visit.querySelector('a').getAttribute('data-href');
    });

    successorLink.href = successorLinkAttr;
    successorLink.setAttribute('data-state', 'active' );
    
});



// Slider ausprobier text 
